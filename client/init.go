...
func (c *ConfigInitializer) initConfig(cluster api.Cluster) error {
	...
	if err := mapstructure.Decode(cluster.Config(), &config); err != nil {
		return err
	}
	...
	for _, key := range keys {
		// TODO: ClusterType shouldn't be in the config file. Remove when it's
		//		 no longer needed.
		if key == "ClusterType" {
			continue
		}

		line := fmt.Sprintf("export %s=\"%v\"", key, config[key])
...
func (c *ConfigInitializer) initSecrets(cluster api.Cluster) error {
	...
	if err := mapstructure.Decode(cluster.Secret(), &secrets); err != nil {
		return err
	}
	...
	lines := []string{}
	for _, key := range keys {
		line := fmt.Sprintf("%s=\"%v\"", key, secrets[key])
		lines = append(lines, line)
	}

	var encryptedLines bytes.Buffer
	plaintextLines := bytes.NewBuffer([]byte(strings.Join(lines, "\n")))
