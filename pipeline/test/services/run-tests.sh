...
docker run -it -v "${CK8S}:${CK8S}" -v "${CONFIG_PATH}:${CONFIG_PATH}" --entrypoint=$CK8S/pipeline/test/services/container-entrypoint.sh -e=CK8S=$CK8S -e=CONFIG_PATH=$CONFIG_PATH elastisys/ck8s-ops:$repoTag